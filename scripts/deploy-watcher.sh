...
build_application() {
    ...
    export HUSKY=0
    # IMPORTANT: systemd sets CI=true which makes npm ci only install production deps
    # Unset ALL npm environment variables that affect installation
    unset CI
    unset NODE_ENV
    ...
    # Use npm install with explicit --production=false to force ALL dependencies
    # This ensures devDependencies are installed even in systemd context
    log_info "Running npm install with --production=false to ensure all dependencies..."
    if ! npm install --production=false --prefer-offline --ignore-scripts; then
        log_warn "First npm install attempt failed, retrying with force..."
        # Try again with force flag to overcome any issues
        if ! npm install --production=false --prefer-offline --force --ignore-scripts; then
            log_error "npm install failed even with --force and --production=false"
            return 1
        fi
    fi
